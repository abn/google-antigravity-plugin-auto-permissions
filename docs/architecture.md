...
* **Safe Session Artifact Fast-Path:** Reading (`view_file`, `list_dir`, `grep_search`) and authoring non-executable session documents (`.md`, `.json`, `.txt`, `.csv`, `.diff`, `.patch`) strictly inside the active session directory (`<session_dir>/...` or `~/.gemini/antigravity/brain/<conversation_id>/...`) are fast-path approved in `0.1ms`. Sensitive perimeter files (`.env`, private keys) and executable scripts (`.sh`, `.py`) inside session directories strictly bypass the fast-path.
